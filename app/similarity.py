...
from utils.db_utils import db_conn  # Import the helper function
...
def plot_similarity_over_time(similarity_df: pd.DataFrame, country_a: str, country_b: str, 
    ...
    window = min(5, len(similarity_df))  # Use smaller window if few data points
...
def run_similarity_analysis(country_a: str, country_b: str, ontology_id: Optional[str] = None, 
                           save_plot: bool = False, output_dir: Optional[Path] = None):
    """
    Run the complete similarity analysis workflow.
    ...
    Returns:
        DataFrame with similarity scores and the matplotlib figure
    """
    try:
        ...
        if similarity_df.empty:
            logging.warning(f"No matching data found for {country_a} vs {country_b}")
        ...
        if save_plot and output_dir:
            ...
            filename = f"similarity_{country_a.replace(' ', '_')}_{country_b.replace(' ', '_')}"
            if ontology_id:
                filename += f"_{ontology_id.replace(' ', '_')}"
            filename += ".png"
            
            output_path = output_dir / filename
        
        plt_fig = plot_similarity_over_time(similarity_df, country_a, country_b, ontology_id, output_path)
        
        return similarity_df, plt_fig
...
def get_available_ontologies():
    """Get a list of all ontology IDs in the database."""
    conn = db_conns
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT DISTINCT ontology_id, concept_names FROM reginsights_clean ORDER BY concept_names")
            ontologies = [(row[0], row[1]) for row in cur.fetchall()]
            return ontologies
    finally:
        conn.close()
